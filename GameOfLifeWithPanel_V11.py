...
class  GameOfLifeProperties(bpy.types.PropertyGroup):
    ...
    BIRTH_CHANCE : FloatProperty(
        name = "BIRTH_CHANCE",
        description = "Chance of spawning a game-cell per grid-cell at initialization",
        default = 0.2,
        min = 0.01,
        max = 0.8
    )
    ...
    COLOR_CHOICE : EnumProperty(
        ...
        items = [
                ...
                ("HIGHLIGHT",  "highlighting", "Uses 'default_material' but also highlights newly created cells in green"),
...
class Worldgrid():
    ...
    def __init__(self, x, y):
        ...
        self.world = np.zeros((y,x), dtype=int)  # world = numpy array of size x,y
...
def render_output(iteration, output_dir, operator, output_file_pattern_string = 'iteration_%d.jpg'):
    """
    Render the current image/state to the given output path. Use "iteration" to define the number behind image name.
    """
    if bpy.data.is_saved:
        bpy.context.scene.render.filepath = os.path.join(output_dir, (output_file_pattern_string % iteration))
